...
namespace Polyphemalus.Content.NPCs
{
	...
	public class polyphemalus : ModNPC
	{
		...
		public ref float hpThreshold => ref NPC.localAI[3];
		...
		public override void OnSpawn(IEntitySource source)
		{
			...
			if (player.dash == 2)
			{
				Main.NewText("Wearing the corpse of their brother has made them angry!");
		...
		{ 
			...
			if (SoC && target.dash != 2)
			{
				SoC = false;
				OnlySoC = false;
				Main.NewText("Showing disrespect the corpse of their brother enraged them even further!");
				return true;
			}
			return false;
		...
		{
			npcLoot.Add(ItemDropRule.Common(ItemID.EyeMask, 1));
